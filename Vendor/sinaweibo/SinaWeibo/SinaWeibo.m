...
- (void)logInInView:(UIView *)view
{
    if ([self isAuthValid])
    {
        ...
        [[SinaWeiboAuthorizeView alloc] initWithAuthParams:params
                                                  delegate:self];
        authorizeView.frame = CGRectMake(authorizeView.frame.origin.x , 0, authorizeView.frame.size.width, authorizeView.frame.size.height);
        
        [authorizeView showInView:view];
        [authorizeView release];
    }
}
