...
- (void)showInView:(UIView *)view
{
    ...
    webView.frame = CGRectMake(0, -(kBorderWidth)*2, self.frame.size.width,
                               self.frame.size.height - (1 + kBorderWidth*2));
    modalBackgroundView.frame = view.frame;
    [modalBackgroundView addSubview:self];
    [view addSubview:modalBackgroundView];
    
    NSLog(@"WebView Frame=%@ modalBackgroundView Frame=%@", NSStringFromCGRect(webView.frame), NSStringFromCGRect(modalBackgroundView.frame));
